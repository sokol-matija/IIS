...
const steps = [
  ...
  {
    name: "Semgrep",
    cmd: "semgrep scan --error --config p/security-audit --config p/typescript --config p/react --config p/owasp-top-ten",
  },
...
function stepResult(success, duration) {
  ...
  if (success) {
    console.log(
      `  ${c.dim}└─${c.reset} ${c.green}${c.bold}${sym.pass} PASSED${c.reset} ${c.dim}(${formatTime(duration)})${c.reset}\n`
    )
  } else {
    console.log(
      `  ${c.dim}└─${c.reset} ${c.red}${c.bold}${sym.fail} FAILED${c.reset} ${c.dim}(${formatTime(duration)})${c.reset}\n`
    )
  }
}
